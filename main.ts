...
// ====================================================
// START: MODIFIED FUNCTION (handleDashboard for Quantity)
// ====================================================
...
async function handleDashboard(user: User): Promise<Response> {
    ...
    const productListHtml = products.map(product => {
        ...
        const maxQuantity = product.isDigital ? product.stock.length : 99; // Max 99 for non-digital items
        ...
        <div class="product-card ${isOutOfStock ? 'out-of-stock' : ''}">
            ...
            ${priceHtml}
            <form method="POST" action="/buy" onsubmit="return checkBalance('${product.name}', ${displayPrice}, ${user.balance}, this.quantity.value);">
                <input type="hidden" name="productId" value="${product.id}">
                
                ${!isOutOfStock ? 
                    `<input type="number" name="quantity" value="1" min="1" max="${maxQuantity}" 
                             style="width: 50%; margin-bottom: 10px; padding: 5px; text-align: center; border: 1px solid #ddd; border-radius: 5px;">`
    ...
    const html = `
        ...
        <script>
            // (FIXED) Updated checkBalance to calculate total price based on quantity
            function checkBalance(itemName, price, balance, quantity) {
                const total = price * quantity;
                if (balance < total) {
                    alert("Insufficient Balance!\\nYou have " + formatCurrency(balance) + " Ks but need " + formatCurrency(total) + " Ks.\\nPlease contact admin for a top-up.");
                    return false; 
                }
                return confirm("Are you sure you want to buy " + quantity + " x " + itemName + " for " + formatCurrency(total) + " Ks?");
            }
...
// ====================================================
// START: MODIFIED FUNCTION (handleBuy for Bulk Purchase)
// ====================================================
...
async function handleBuy(formData: FormData, username: string): Promise<Response> {
    ...
    const quantity = quantityStr && !isNaN(parseInt(quantityStr)) ? parseInt(quantityStr) : 1; 

    if (!productId || quantity < 1) { 
        return renderMessagePage("Error", "Invalid item ID or quantity.", true);
    }
    ...
    const totalPrice = price * quantity; // <-- NEW: Total price calculation
    const item = product.name;
    let itemDetails: string[] = []; // <-- NEW: Array to hold multiple codes

    const user = await getUserByUsername(username);
    // <-- NEW check against totalPrice
    if (!user || user.balance < totalPrice) { 
        const message = `You have ${formatCurrency(user?.balance ?? 0)} Ks but need ${formatCurrency(totalPrice)} Ks to buy ${quantity} units.`;
        return renderMessagePage("Insufficient Balance", message, true);
    }

    if (product.isDigital) {
        // <-- NEW stock check for bulk quantity
        if (!product.stock || product.stock.length < quantity) { 
            return renderMessagePage("Error", `Sorry, only ${product.stock.length} units left in stock. You requested ${quantity}.`, true);
        }
        
        itemDetails = product.stock.slice(0, quantity); // <-- NEW: Take QUANTITY items
        const newStock = product.stock.slice(quantity); // <-- NEW: Remove QUANTITY items
    ...
    const success = await updateUserBalance(username, -totalPrice); // <-- Deduct totalPrice

    if (success) {
        const detailsString = itemDetails.join('\n'); // Join the codes/details
        // Log bulk purchase details
        await logTransaction(username, -totalPrice, "purchase", `${item} (${quantity} units)`, detailsString); 
        ...
        if (detailsString) {
            // Display purchased codes in a neat block
            const detailsHtml = itemDetails.map(d => `<code style="display: block; font-size: 1.1em; color: #d63384; margin-bottom: 5px;">${d}</code>`).join('');
            detailsMessage = `<br><br>Your purchased item details (${quantity} units):<br><div style="background:#fff; padding:10px; border-radius:5px; border: 1px dashed #ddd;">${detailsHtml}</div>`;
            reminderMessage = "<br><br><small>This page will auto-redirect. You can view these codes again in your 'My Info' page.</small>";
        }
        
        const message = `You bought <strong>${item}</strong> (${quantity} units) for ${formatCurrency(totalPrice)} Ks.<br>Your new balance is <strong>${formatCurrency(newBalance)} Ks</strong>.${detailsMessage}${reminderMessage}`;
...
function handleLogout(): Response {
    ...
    // Must match ALL attributes of the cookie to delete it
