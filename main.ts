...
// NEW: TopupRequest Interface
interface TopupRequest {
    id: string; // Unique ID (timestamp)
    username: string;
    amount: number;
    paymentMethod: string;
    screenshotUrl: string; // Admin စစ်ဆေးရန် ပုံ Url
    status: "pending" | "approved" | "rejected";
    requestedAt: string;
    handledBy?: string; // Admin username
}
...
async function getSalesSummary(): Promise<{ totalUsers: number, totalRevenue: number, totalSales: number }> {
    ...
    let totalUsers = 0;
    let totalRevenue = 0; // Top-ups, Vouchers, Admin Credit
    let totalSales = 0;   // Purchases (Items/Transfers)
    ...
    for await (const entry of allTransactionsEntries) {
        ...
        // Skip rolled back transactions for clean reporting
        if (t.isRolledBack) {
            continue;
        }
        ...
        if (t.type === "topup" && t.amount > 0) {
            ...
            // Purchases/Transfers are stored as negative numbers
            ...
            document.querySelectorAll('.accordion-header').forEach(header => {
                header.addEventListener('click', () => {
                    ...
                    document.querySelectorAll('.accordion-content').forEach(otherContent => {
                        // FIX: Corrected syntax for document.querySelector in template literal
