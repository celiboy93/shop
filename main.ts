...
// NEW: Hashing Utilities (Using native Web Crypto API for simplicity)
...
async function verifyPassword(inputPassword: string, storedHash: string): Promise<boolean> {
    // If the stored hash is the old plain text (which starts with a letter, not a full SHA-256 hex string), 
    // fall back to simple string comparison for migration purposes. 
    // (This is only safe if you start hashing immediately after this deploy)
