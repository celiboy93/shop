...
const PWA_MANIFEST_JSON = {
    ...
    "icons": [
        {
            "src": "https://i.imgur.com/gzaeXGk.png", // 192x192 icon (ဥပမာ)
            "sizes": "192x192",
            "type": "image/png"
        },
        {
            "src": "https://i.imgur.com/gzaeXGk.png", // 512x512 icon (ဥပမာ)
            "sizes": "512x512",
            "type": "image/png"
        }
    ]
...
// App ဖွင့်ဖွင့်ချင်း ကြိုဆွဲထားမယ့် အဓိက file များ
const URLS_TO_CACHE = [
    '/dashboard',
    '/user-info',
    '/login'
...
self.addEventListener('fetch', event => {
    event.respondWith(
        caches.match(event.request)
            .then(response => {
                // Cache ထဲမှာ တွေ့ရင် Cache ကနေ ပြန်ပေးမယ်
                if (response) {
                    return response;
                }
                // Cache ထဲမှာ မတွေ့ရင် အင်တာနက်ကနေ သွားယူမယ်
...
async function transferBalance(senderUsername: string, recipientUsername: string, amount: number): Promise<string> {
    ...
    // 1. Log အတွက် လိုအပ်တာများ ကြိုပြင်ဆင်ပါ
    const senderTimestamp = new Date().toISOString();
    // (တပြိုင်တည်းမဖြစ်အောင် 1 millisecond ခွာလိုက်ပါ)
    ...
    // 2. Log object တွေကို ကြိုဆောက်ထားပါ
    ...
    // 3. (while(true) loop) သည် race condition များအတွက် မှန်ကန်ပါသည်
    while (true) {
        ...
        // 4. (FIXED) kv.atomic() ထဲတွင် Log ရေးခြင်းကိုပါ ထည့်သွင်းလိုက်ပါ
        const res = await kv.atomic()
            .check(senderResult) // Sender data မပြောင်းသေးကြောင်း စစ်ဆေးပါ
            .check(recipientResult) // Recipient data မပြောင်းသေးကြောင်း စစ်ဆေးပါ
            .set(senderKey, { ...sender, balance: newSenderBalance }) // Sender ပိုက်ဆံနုတ်ပါ
            .set(recipientKey, { ...recipient, balance: newRecipientBalance }) // Recipient ပိုက်ဆံပေါင်းပါ
            .set(senderLogKey, senderTransaction) // Sender Log ရေးပါ
            .set(recipientLogKey, recipientTransaction) // Recipient Log ရေးပါ
            .commit();
        
        if (res.ok) {
            // အားလုံး (၄) ခုလုံး အောင်မြင်မှ "success" ပြန်ပါမည်
            return "success"; 
        }
        // res.ok မဟုတ်ခဲ့လျှင် (တခြားသူနှင့် တပြိုင်တည်း ဖြစ်လျှင်) loop ပြန်ပတ်ပါမည်
    }
}
...
async function handleDashboard(user: User): Promise<Response> {
    ...
    const html = `
        ...
        <style>${globalStyles}
            ...
            /* (FIXED) ပုံမပေါ်ခင် နေရာဦးထားမယ့် ပုံစံ */
        ...
        <script>
            document.addEventListener("DOMContentLoaded", function() {
                ...
                lazyImages.forEach(function(img) {
                    // data-src ထဲက URL ကို အစစ်အမှန် src ထဲ ထည့်ပါ
                    img.src = img.dataset.src;
                    
                    // load ဖြစ်သွားရင် class ကို ဖယ်ပါ
...
async function handler(req:Request): Promise<Response> {
    try { // (NEW) Global error catcher
        ...
        if (req.method === "GET") {
            ...
            // ===========================================
            // START: PWA FILES (ဒါက PWA အတွက်ပါ)
            // ===========================================
