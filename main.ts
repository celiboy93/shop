...
async function transferBalance(senderUsername: string, recipientUsername: string, amount: number): Promise<string> {
    ...
    while (true) {
        ...
        if (res.ok) {
            // NEW: Update sender's spend and tier
            await updateUserSpendAndTier(senderUsername, amount); // Transfers count as 'spending' for sender
...
// NEW: Function to get Admin-initiated Top-up/Credit history
async function getAdminTopupHistory(): Promise<Transaction[]> {
    const entries = kv.list<Transaction>({ prefix: ["transactions"] });
    const logs: Transaction[] = [];
    for await (const entry of entries) {
        const t = entry.value;
        // Check for 'topup' type and specific item names used by admin (Admin Top-Up, Voucher, Global Bonus, ROLLBACK purchase)
        if (t.type === 'topup' && (t.itemName?.includes('Admin Top-Up') || t.itemName?.includes('Voucher:') || t.itemName?.includes('Bonus') || t.itemName?.includes('ROLLBACK/PURCHASE'))) {
            // NOTE: Need to fetch the username from the Key since the transaction object itself doesn't store it
            const username = entry.key[1] as string;
            // Add username back to log item for display
            logs.push({ ...t, itemName: `${t.itemName} to ${username}` }); 
        }
    }
...
async function addProduct(name: string, price: number, salePrice: number | null, imageUrl: string, isDigital: boolean, stock: string[], category: string): Promise<boolean> {
    const id = Date.now().toString(); 
    const product: Product = { id, name, price, salePrice, imageUrl, isDigital, stock: stock || [], category };
    const key = ["products", id];
    const res = await kv.set(key, product);
    return res.ok;
}

async function updateProduct(id: string, name: string, price: number, salePrice: number | null, imageUrl: string, isDigital: boolean, stock: string[], category: string): Promise<boolean> {
    const key = ["products", id];
    const product: Product = { id, name, price, salePrice, imageUrl, isDigital, stock: stock || [], category };
    const res = await kv.set(key, product);
    return res.ok;
...
async function renderAdminPanel(token: string, message: string | null): Promise<Response> {
    ...
    const adminTopups = await getAdminTopupHistory();
        ...
        <body><div class="container" style="max-width: 700px;">
            ...
            <div class="accordion-content" id="section4">
                ...
                <div class="history-list">
                    ${adminTopupHistoryHtml.length > 0 ? adminTopupHistoryHtml : '<p>No administrative credit movements found.</p>'}
                </div>
            </div>
...
async function handleDashboard(req: Request, user: User): Promise<Response> {
    ...
    const productListHtml = products.map(product => {
        ...
        <div class="product-card ${isOutOfStock ? 'out-of-stock' : ''}">
            ...
            <span style="font-size: 0.9em; color: #555;">Tier: ${product.category}</span>
            ${priceHtml}
...
async function handleUserInfoPage(req: Request, user: User): Promise<Response> {
    const allTransactions = await getTransactions(user.username);
...
async function handleAddProduct(formData: FormData): Promise<Response> {
    ...
    await addProduct(name, price, salePrice, imageUrl, isDigital, stock, category);
...
async function handleUpdateProduct(formData: FormData): Promise<Response> {
    ...
    await updateProduct(productId, name, price, salePrice, imageUrl, isDigital, stock, category);
