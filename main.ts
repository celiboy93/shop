...
// NEW: Function to get Admin-initiated Top-up/Credit history (Excluding Bonuses)
async function getAdminTopupHistory(): Promise<Transaction[]> {
    ...
    for await (const entry of entries) {
        ...
        if (t.type === 'topup' && isAdminCredit) {
            // Note: We need to manually add the username back from the key for display
        ...
        <body><div class="container" style="max-width: 700px;">
            ...
            <div class="accordion-content" id="section4">
                ...
                <form method="GET" class="admin-history-filter" action="/admin/panel">
                    <input type="hidden" name="token" value="${token}">
                    <input type="text" name="history_search" placeholder="Search user/item..." value="${new URL(req.url).searchParams.get('history_search') || ''}">
...
async function handleBuy(formData: FormData, username: string): Promise<Response> {
    ...
    // NEW LOGIC: Only check balance if price > 0
...
async function handleUpdateProduct(formData: FormData): Promise<Response> {
    ...
    const salePriceStr = formData.get("sale_price")?.toString();
    // FIXED: Convert salePrice to number. If it's empty, use null. If it's '0', keep 0.
    ...
    if (!productId || !name || isNaN(price) || price < 0 || !imageUrl) { // Price >= 0 is allowed now
