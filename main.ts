...
async function handleUserInfoPage(req: Request, user: User): Promise<Response> {
    ...
    const allPurchases = filteredTransactions.filter(t => t.type === 'purchase');
    ...
    // --- History HTML ---
    const topUpHistory = filteredTransactions.filter(t => t.type === 'topup')
    ...
    const html = `
        ...
        <div class="history">
            ...
            <div class="history-list">
                ${filteredTransactions.length > 0 ? `<ul>${topUpHistory}${purchaseHistory}</ul>` : `<p>No transactions found matching your criteria.</p>`}
            </div>
        </div>
