...
const PWA_MANIFEST_JSON = {
    "name": "Celiboy Shop",
    "short_name": "Shop",
    "description": "A simple shop built with Deno.",
    "start_url": "/dashboard",
    "display": "standalone",
    ...
    "icons": [
        {
            "src": "https://i.imgur.com/gzaeXGk.png", // 192x192 icon (ตัวอย่าง)
            "sizes": "192x192",
            "type": "image/png"
        },
        {
            "src": "https://i.imgur.com/gzaeXGk.png", // 512x512 icon (ตัวอย่าง)
            "sizes": "512x512",
            "type": "image/png"
        }
    ]
...
const PWA_SERVICE_WORKER_JS = `
const CACHE_NAME = 'celiboy-shop-cache-v1';
// ไฟล์หลักที่จะโหลดเก็บไว้ล่วงหน้า
const URLS_TO_CACHE = [
    '/dashboard',
    '/user-info',
    '/login'
...
self.addEventListener('fetch', event => {
    event.respondWith(
        caches.match(event.request)
            .then(response => {
                // ถ้าเจอใน Cache ให้ส่งจาก Cache
                if (response) {
                    return response;
                }
...
async function transferBalance(senderUsername: string, recipientUsername: string, amount: number): Promise<string> {
    ...
    // 1. เตรียมข้อมูลสำหรับ Log ล่วงหน้า
    const senderTimestamp = new Date().toISOString();
    // (ขยับเวลา 1 มิลลิวินาที เพื่อไม่ให้ Key ซ้ำกัน)
    ...
    // 2. สร้าง object ของ Log ไว้ล่วงหน้า
    ...
    // 3. ใช้ (while(true) loop) เพื่อจัดการกับ race conditions
    while (true) {
        ...
        // 4. (FIXED) ทำทุกอย่าง (หักเงิน, เพิ่มเงิน, บันทึก Log 2 อัน) ภายใน atomic เดียว
        const res = await kv.atomic()
            .check(senderResult) // ตรวจสอบว่าข้อมูล Sender ไม่เปลี่ยนแปลง
            .check(recipientResult) // ตรวจสอบว่าข้อมูล Recipient ไม่เปลี่ยนแปลง
            .set(senderKey, { ...sender, balance: newSenderBalance }) // หักเงิน Sender
            .set(recipientKey, { ...recipient, balance: newRecipientBalance }) // เพิ่มเงิน Recipient
            .set(senderLogKey, senderTransaction) // เขียน Log ของ Sender
            .set(recipientLogKey, recipientTransaction) // เขียน Log ของ Recipient
            .commit();
        
        if (res.ok) {
            // เมื่อทุกอย่าง (ทั้ง 4 ขั้นตอน) สำเร็จ ถึงจะ return "success"
            return "success"; 
        }
        // ถ้า res.ok เป็น false (เกิด race condition) loop จะวนกลับไปทำใหม่
    }
}
...
function renderLoginForm(req: Request): Response {
    ...
    const html = `<!DOCTYPE html><html><head>
        ...
        <meta name="theme-color" content="${PWA_MANIFEST_JSON.theme_color}">
        ...
        <p style="margin-top:20px; text-align:center;">Don't have an account? <a href="/register">Register Here</a></p></div>
...
function renderRegisterForm(req: Request): Response {
    ...
    const html = `<!DOCTYPE html><html><head>
        ...
        <p style="margin-top:20px; text-align:center;">Already have an account? <a href="/login">Login</a></p></div>
...
// ====================================================
// START: MODIFIED FUNCTION (handleDashboard)
// ====================================================
...
async function handleDashboard(user: User): Promise<Response> {
    ...
    const productListHtml = products.map(product => {
        ...
        <div class="product-card ${isOutOfStock ? 'out-of-stock' : ''}">
            ...
            ${product.imageUrl.startsWith('http') 
                ? `<img data-src="${product.imageUrl}" alt="${product.name}" class="product-image lazy-load">` 
    ...
    const html = `
        ...
        <style>${globalStyles}
            ...
            /* (FIXED) เพิ่ม Style สำหรับรูปที่ยังไม่โหลด */
            .product-image.lazy-load {
                background-color: #eee; /* สี Placeholder */
            }
        </style>
        </head>
        ...
        <script>
            document.addEventListener("DOMContentLoaded", function() {
                ...
                lazyImages.forEach(function(img) {
                    // คัดลอก URL จาก data-src ไปยัง src จริง
                    img.src = img.dataset.src;
                    
                    // เมื่อโหลดเสร็จ ให้ลบคลาสออก
...
async function handler(req:Request): Promise<Response> {
    try { // (NEW) Global error catcher
        ...
        if (req.method === "GET") {
            ...
            // ===========================================
            // START: PWA FILES (เพิ่มส่วนนี้)
            // ===========================================
            if (pathname === "/manifest.json") {
                return new Response(JSON.stringify(PWA_MANIFEST_JSON), {
                    headers: { "Content-Type": "application/json" }
                });
            }
            if (pathname === "/sw.js") {
                return new Response(PWA_SERVICE_WORKER_JS, {
        ...
        if (req.method === "POST") {
            ...
            // Public POST
            if (pathname === "/auth") return await handleAuth(formData); // handleAuth also needs to check bonus
