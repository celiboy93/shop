...
async function addProduct(name: string, price: number, salePrice: number | null, imageUrl: string, isDigital: boolean, stock: string[], category: string): Promise<boolean> {
    ...
    const product: Product = { id, name, price, salePrice, imageUrl, isDigital, stock: stock || [], category, isSharedStock: false };
    const key = ["products", id];
...
async function updateProduct(id: string, name: string, price: number, salePrice: number | null, imageUrl: string, isDigital: boolean, stock: string[], category: string): Promise<boolean> {
    const key = ["products", id];
    const product: Product = { id, name, price, salePrice, imageUrl, isDigital, stock: stock || [], category, isSharedStock: false }; // isSharedStock left as false as it was not in the form
    const res = await kv.set(key, product);
    return res.ok;
}
...
async function handleBuy(formData: FormData, username: string): Promise<Response> {
    ...
    if (product.isDigital) {
        ...
        // NEW LOGIC: Shared/Unlimited Stock Handling
        const itemToSell = product.stock[0]; 
        let newStock = product.stock;
        
        // Only remove stock if it's NOT shared stock and stock count > 1
        if (!product.isSharedStock || product.stock.length > 1) { 
