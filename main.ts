...
async function handleAuth(formData: FormData): Promise<Response> {
    ...
    if (bonus && bonus.isActive && !user.receivedBonus) {
        ...
        // If it fails (race condition), they just log in normally and will will get it next time.
    }
...
async function handleBuy(formData: FormData, username: string): Promise<Response> {
    ...
    if (product.isDigital) {
        ...
        const itemToSell: string = product.stock[0]; 
        let newStock = product.stock;
        
        // NEW SHARED/RANDOM LOGIC: 
        if (product.isSharedStock) { 
             // Randomly select one item, but DO NOT remove it from stock
             const randomIndex = Math.floor(Math.random() * product.stock.length);
             itemToSell = product.stock[randomIndex];
             newStock = product.stock; // Keep stock unchanged for shared items
        } else {
             // Private Stock: Remove the first item
             itemToSell = product.stock[0]; 
...
async function handleAddProduct(formData: FormData): Promise<Response> {
    ...
    const isSharedStock = formData.get("isSharedStock") === "on";
