...
interface Transaction {
    ...
    itemName?: string; 
    itemDetails?: string; 
    isRolledBack?: boolean; // NEW: Rollback status
}
...
// NEW: Helper function to get a specific transaction by key parts
...
async function handleRefundRollback(username: string, timestamp: string, adminUsername: string): Promise<string> {
    ...
    // Purchase: amount is negative (e.g., -5000). Refund amount is positive (+5000).
    // Topup: amount is positive (e.g., 10000). Rollback amount is negative (-10000).
    const rollbackAmount = transaction.type === "purchase" ? Math.abs(transaction.amount) : -transaction.amount; 
    ...
    if (!updateOriginalRes.ok) {
        ...
        // Note: Logic continues, but the original transaction status might be inconsistent.
    }
...
async function handleUserInfoPage(req: Request, user: User): Promise<Response> {
    ...
    // NEW: Combined history with rollback status and copyable key
    ...
    const transactionHistoryItems = nonDigitalTransactions.map(t => {
        ...
        // --- NEW LOGIC: Only show Copy Key button for un-rolledback PURCAHSE/TRANSFER logs ---
        let copyButtonHtml = '';
        if (t.type === 'purchase' && !t.isRolledBack) { // Includes purchases and transfers
             copyButtonHtml = `
