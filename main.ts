// main.ts (v2.1 - With User Change Password Feature)
...
// 1. Create a secure session in KV
async function createSession(username: string, remember: boolean): Promise<string> {
    ...
    const expireIn = remember ? 1000 * 60 * 60 * 24 * 30 : 1000 * 60 * 60; // 30 days or 1 hour
    
    // Store: key=["sessions", token] -> value=username
    await kv.set(["sessions", token], username, { expireIn });
    return token;
}

// 2. Get Username from Session Token
async function getUsernameFromSession(req: Request): Promise<string | null> {
    ...
    try {
        ...
        // Lookup token in KV to find username
        const result = await kv.get<string>(["sessions", token]);
        return result.value; // Returns username or null if expired/invalid
    } catch (e) {
    ...
    const html = `
        ...
        <body><div class="container" style="max-width: 700px;">
            ...
            <div class="accordion-content" id="section4">
                ...
                <h2>⚠️ Testing Data Cleanup</h2>
                <button type="button" class="reset" onclick="confirmCleanup()">DELETE ALL USERS & HISTORY</button>
            </div>
        ...
        <form id="cleanupForm" method="POST" action="/admin/cleanup">
            <input type="hidden" name="token" value="${token}">
        </form>
        <script>
            function confirmCleanup() {
                if (confirm("WARNING: Are you sure you want to DELETE ALL USER ACCOUNTS AND HISTORY? This cannot be undone.")) {
                    if (prompt("To proceed, type the word 'DELETE' (case sensitive):") === 'DELETE') {
...
async function handleUserInfoPage(req: Request, user: User): Promise<Response> {
    ...
    if (message === "password_changed") messageHtml = `<div class="success-msg">Success! Your password has been changed.</div>`;
    ...
    const html = `
        ...
        <style>${globalStyles}
            ...
            .form-box button.password { background-color: #6c757d; }
        ...
        <div class="form-box">
            ...
            <form action="/change_password" method="POST">
                <label>Old Password:</label><input type="password" name="old_password" required style="width: 95%;">
                <label style="margin-top: 10px; display: block;">New Password:</label><input type="password" name="new_password" required style="width: 95%;">
                <button type="submit" class="password" style="width: 100%; margin-top: 15px;">Update Password</button>
            </form>
        </div>
...
async function handleAuth(formData: FormData): Promise<Response> {
    ...
    if (!username || !password) {
        return new Response("Redirecting...", { status: 302, headers: { "Location": "/login?error=missing" } });
    }
    ...
    if (user.isBlocked) {
        return new Response("Redirecting...", { status: 302, headers: { "Location": "/login?error=blocked" } });
    }
    ...
    // Bonus check logic (simplified here as it's also in authenticateUser, but safe to keep)
    const bonus = await getGlobalBonus();
    if (bonus && bonus.isActive && !user.receivedBonus) {
        ...
        const res = await kv.atomic().check(userResult).set(userKey, updatedUser).commit();
        if (res.ok) {
            await logTransaction(username, bonus.amount, "topup", "Event Bonus");
        }
    }
    
    // SECURITY FIX: Create Session Token
    const token = await createSession(username, remember);
...
async function handleRegister(formData: FormData): Promise<Response> {
    ...
    if (success) {
        ...
        return new Response("User exists. Redirecting...", { status: 302, headers: { "Location": "/register?error=exists" } });
    }
}
...
async function handleAdminCleanup(formData: FormData): Promise<Response> {
    const token = formData.get("token")?.toString();
    const adminBackLink = `/admin/panel?token=${token}`;
    
    // Safety check is handled by client-side JS confirm/prompt

    const message = await deleteAllUsersAndRelatedData();
...
// NEW: Handler for User Change Password
async function handleChangePassword(formData: FormData, username: string): Promise<Response> {
    ...
    if (!oldPassword || !newPassword) {
        headers.set("Location", `/user-info?error=${encodeURIComponent("Both fields are required.")}`);
        return new Response("Redirecting...", { status: 302, headers });
    }
    
    const result = await changeUserPassword(username, oldPassword, newPassword);
    
    if (result === "success") {
        headers.set("Location", `/user-info?message=password_changed`);
    } else {
        headers.set("Location", `/user-info?error=${encodeURIComponent(result)}`);
    }
    return new Response("Redirecting...", { status: 302, headers });
...
async function handleLogout(req?: Request): Promise<Response> {
    if (req) {
        await invalidateSession(req); // Remove from KV
    }
...
async function authenticateUser(req: Request): Promise<User | null> {
    // NEW: Get username via secure session lookup
...
async function handler(req:Request): Promise<Response> {
    try { // Global error catcher
        ...
        if (req.method === "GET") {
            ...
            if (pathname === "/logout") return await handleLogout(req);
        ...
        if (req.method === "POST") {
            ...
            if (user) {
                ...
                if (pathname === "/change_password") return await handleChangePassword(formData, user.username); // NEW
            } else if (pathname === "/buy" || pathname === "/redeem_voucher" || pathname === "/transfer_funds" || pathname === "/change_password") {
                return await handleLogout(req); // Not logged in or blocked, redirect
            }
