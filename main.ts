...
async function handleUserInfoPage(req: Request, user: User): Promise<Response> {
    ...
    const nextUrl = nextCursor ? `/user-info?cursor=${nextCursor}&filter_type=${filterType}&search=${searchTerm}` : '#';

    let prevUrl = '#';
    // NOTE: Implementing "previous" page is complex with forward-only cursors. 
    // We'll keep it simple for now and rely on "Next Page" functionality.

    const paginationHtml = `
        <div style="display: flex; justify-content: center; gap: 10px; margin-top: 15px;">
            ${/* NOTE: Previous Button implementation is complex with KV cursor, so we disable it for simplicity */''}
            <button disabled style="width: 120px; background-color: #ccc; cursor: not-allowed;">Previous Page</button>
    ...
    const html = `
        ...
        <div class="history">
            ...
            ${nextCursor ? `<a href="/user-info?cursor=${nextCursor}&filter_type=${filterType}&search=${searchTerm}" style="display:block; text-align:center; margin-top:10px;">Next Page (10 more)</a>` : ''}
