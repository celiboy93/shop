...
const PWA_MANIFEST_JSON = {
    ...
    "icons": [
        {
            "src": "https://i.imgur.com/gzaeXGk.png", // 192x192 icon (Example)
            "sizes": "192x192",
            "type": "image/png"
        },
        {
            "src": "https://i.imgur.com/gzaeXGk.png", // 512x512 icon (Example)
            "sizes": "512x512",
            "type": "image/png"
        }
    ]
...
self.addEventListener('fetch', event => {
    event.respondWith(
        caches.match(event.request)
            .then(response => {
                // Found in cache, return from cache
                if (response) {
                    return response;
                }
    ...
    try { // Global error catcher
        ...
        if (req.method === "GET") {
            ...
            if (!user) {
                ...
                // (FIXED) Only redirect to logout if accessing protected routes
                if(pathname === "/" || pathname === "/dashboard" || pathname === "/user-info") {
                    return handleLogout();
                }
                // If they are not logged in but on a public page (like /login), do nothing here
            } else {
        ...
        // (FIXED) Only redirect if the path is not a recognized public path
